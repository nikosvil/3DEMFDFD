...
function [FWD_IP,FWD_QP,Hn] = FDEM1DFWD_RC1(S,M)
...
%         S.r= sqrt(S.x^2 + S.y^2 + (S.z+S.height)^2);                       % Coil spacing (m)
                S.r= sqrt(S.x^2 + S.y^2 );                       % Coil spacing (m) % modified by nikos
...
% Total field
                H_rsp=  H+Hn;  % modified by nikos

    
        
    %
    % Get forward response of totala field
    %
...
function [H, Hn] = magneticFields(S,M)
    ...
    %
        ...
        if strcmpi(S.ori,'ZZ')
            ...
            % Calculate Hzz (HCP)
            %
            if S.z<=0 % added by nikos
                if abs(S.z)<=S.height % added by nikos
                H= S.mom/(4*pi)*digitalFilter('rTEp2',0,S,M,mu0,eps0); % added by nikos
                Hn= S.mom/(4*pi)*digitalFilter('rTE02',0,S,M,mu0,eps0);
                else
                    H= S.mom/(4*pi)*digitalFilter('rTEp2',0,S,M,mu0,eps0);% added by nikos
                Hn= S.mom/(4*pi)*digitalFilter('rTE002',0,S,M,mu0,eps0);% added by nikos
                end
            else
                H=0;Hn=0;% added by nikos
            end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        elseif strcmpi(S.ori,'ZZE')% added by nikos H = E
                       if S.z<=0
                          if abs(S.z)<=S.height
                H= -j*2*pi*S.freq*mu0*S.mom/(4*pi)*digitalFilter('rTEp2E',1,S,M,mu0,eps0);% added by nikos
                Hn= -j*2*pi*S.freq*mu0*S.mom/(4*pi)*digitalFilter('rTE02E',1,S,M,mu0,eps0);% added by nikos
                          else
                           H= -j*2*pi*S.freq*mu0*S.mom/(4*pi)*digitalFilter('rTEp2E',1,S,M,mu0,eps0);% added by nikos
                           Hn= -j*2*pi*S.freq*mu0*S.mom/(4*pi)*digitalFilter('rTE002E',1,S,M,mu0,eps0);% added by nikos
                          end
                       else
                           H= -j*2*pi*S.freq*mu0*S.mom/(4*pi)*digitalFilter('rTEpp2E',1,S,M,mu0,eps0);% added by nikos
                           Hn=0;
...
function y = digitalFilter(funname,order,S,M,mu0,eps0)
    ...
    %
        ...
        elseif strcmp(funname,'rTE02')% below trans H
        ...
        elseif strcmp(funname,'rTEp2')
            ...
            elseif strcmp(funname,'rTEp2E')% added by nikos above the E
            YF = rTEp2E(lambda,S,M,mu0,eps0);
            elseif strcmp(funname,'rTEpp2E')% added by nikos below surface for E
            YF = rTEpp2E(lambda,S,M,mu0,eps0);
            elseif strcmp(funname,'rTE002')% added by nikos above trans H
            YF = rTE002(lambda,S,mu0,eps0);
            elseif strcmp(funname,'rTE002E')% added by nikos above trans E
            YF = rTE002E(lambda,S,mu0,eps0);
            elseif strcmp(funname,'rTE02E')% added by nikos below trans E
...
if S.z<=0
                ...
                rTEbelow=(2.*Y0)./(Y0+Yhat(:,1)); % for below the earth surface added by nikos for E% added by nikos
                rTE=rTEbelow;% added by nikos
...
%                 y= (exp(-u0.*(S.z+S.height))).*lambda.^2./u0; % for elec below the transmitter and above the earth
...
%                 y= (rTE.*exp(u0.*(S.z-S.height))).*lambda.^2./u0; % FOR Elec nelow the transmitter and above the Earth
...
%         y= (rTE.*exp(u0.*(S.z-S.height))).*lambda.^3./u0;
                y= (rTE.*exp((-u1.*S.z-u0.*S.height))).*lambda.^2./u0;; % FOR Elec below the transmitter and below the Earth

    
end

function y = rTE002(lambda,S,mu0,eps0)
    % added by nikos
    %
    % Define variable u0
    %
