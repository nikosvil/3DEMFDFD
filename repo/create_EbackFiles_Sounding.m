...
l1x=A(9,1); l2x=A(9,2);
l1y=A(10,1);l2y=A(10,2);
l1z=A(11,1);l2z=A(11,2);
    ...
    for iy=1:ny-1
        ...
        for ix=1:nx
            xcounter=ix*hx;
             k=k+1;
             %if (zcounter<=zz)
                 if (zcounter>l1z) && (zcounter<l2z) && (ycounter>l1y) && (ycounter<l2y) && (xcounter>l1x) && (xcounter<l2x)
                     [~, ETX(k,1)]=E_to_onepoint_hankel_transform([ix*hx-hx/2 iy*hy zz ], posTx, posTy, posTz, sigma2, f);
                 else
                     [~, ETX(k,1)]=E_to_onepoint_hankel_transform([ix*hx-hx/2 iy*hy zz ], posTx, posTy, posTz, sigma1, f);
                 end
             %end
        end
    end
...
for iz=1:nz-1
    ...
    for iy=1:ny
        ...
        for ix=1:nx-1
            ...
            %if (zcounter<=zz) 
             if (zcounter>l1z) && (zcounter<l2z) && (ycounter>l1y) && (ycounter<l2y) && (xcounter>l1x) && (xcounter<l2x)
              [ETY(k,1), ~]=E_to_onepoint_hankel_transform([ix*hx iy*hy-hy/2 zz], posTx, posTy, posTz, sigma2, f);
             else
              [ETY(k,1), ~]=E_to_onepoint_hankel_transform([ix*hx iy*hy-hy/2 zz], posTx, posTy, posTz, sigma1, f);
             end
            %end
         end
    end
