...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  [FWD_IP,FWD_QP] = FDEM1DFWD_RC1(S,M)
%
%  Use:
%  Calculates the forward response of a given half-space and
...
%
%  And the scipt created by Daan Hanssens,Ghent University, Belgium
...
function [FWD_IP,FWD_QP,En] = FDEM1DFWD_Efields(S,M)

                S.r= sqrt(S.x^2 + S.y^2 );                       % Coil spacing (m) % 
    ...
    %
    % Get forward response of total field
    %
...
%
%  Use:
%  Calculate electric fields for Z-directed transmitter 
%  and Z-directed receivers,'ZZE').
%
%  Input:
%  S (structure)           Sensor characteristics
%  M (structure)           Model characteristics
...
function [E, En] = electricFields(S,M)
    ...
    %
        ...
        if strcmpi(S.ori,'ZZE')

          
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       
                       if S.z<=0% receiver above earths surface
                          if abs(S.z)<=S.height% receiver above earhts surface but below transmitter
                           E= -j*2*pi*S.freq*mu0*S.mom/(4*pi)*digitalFilter('rTEp2E',1,S,M,mu0,eps0);% above the Earth
                           En= -j*2*pi*S.freq*mu0*S.mom/(4*pi)*digitalFilter('rTE02E',1,S,M,mu0,eps0);% below transmitter
                          else% receiver above earhts surface and above transmitter
                           E= -j*2*pi*S.freq*mu0*S.mom/(4*pi)*digitalFilter('rTEp2E',1,S,M,mu0,eps0);% above the Earth
                           En= -j*2*pi*S.freq*mu0*S.mom/(4*pi)*digitalFilter('rTE002E',1,S,M,mu0,eps0);% above transmitter
                          end
                       else% receiver below earths surface
                           E= -j*2*pi*S.freq*mu0*S.mom/(4*pi)*digitalFilter('rTEpp2E',1,S,M,mu0,eps0);% below the Earth
                           En=0;
...
function y = digitalFilter(funname,order,S,M,mu0,eps0)
    ...
    %
        ...
        if strcmp(funname,'rTEp2E')% above the Earth
            YF = rTEp2E(lambda,S,M,mu0,eps0);
            elseif strcmp(funname,'rTEpp2E')% below the Earth
            YF = rTEpp2E(lambda,S,M,mu0,eps0);
            elseif strcmp(funname,'rTE002E')% above transmitter
            YF = rTE002E(lambda,S,mu0,eps0);
            elseif strcmp(funname,'rTE02E')% below transmitter
            YF = rTE02E(lambda,S,mu0,eps0);
        end
...
if S.z<=0
                ...
                rTE=(lambda-u(:,1))./(lambda+u(:,1));
else
                rTEbelow=(2.*u)./(lambda+u(:,1)); % for below the earth surface 
                rTE=rTEbelow;
                
end
        else
             error('Imported parameters not corresponding to a half-space.');
...
function y = rTEp2E(lambda,S,M,mu0,eps0)
    ...
    %
                ...
                y= (rTE.*exp(u0.*(S.z-S.height))).*lambda.^2./u0; % For E above the Earth

    
end

function y = rTEpp2E(lambda,S,M,mu0,eps0)
    ...
    %
                ...
                y= (rTE.*exp((-u1.*S.z-u0.*S.height))).*lambda.^2./u1;; % For E below the transmitter and below the Earth

   
end

function y = rTE002E(lambda,S,mu0,eps0)
    ...
    %
                ...
                y= (exp(u0.*(S.z+S.height))).*lambda.^2./u0; % For E above the transmitter and above the Earth

    
end

function y = rTE02E(lambda,S,mu0,eps0)
    ...
    %
                ...
                y= (exp(-u0.*(S.z+S.height))).*lambda.^2./u0; % For E below the transmitter and above the earth
